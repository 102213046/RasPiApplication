...
char pin[]="0000";
char apn[]="internet";//電信公司
char user_name[]="";
char password[]="";

char url[ ]="210.241.199.199";
...
void setup(){
    ...
    //sets the PIN code
    snprintf(aux_str, sizeof(aux_str), "AT+CPIN=%s", pin);//snprintf:字串放進字元陣列
    sendATcommand(aux_str, "OK", 2000);

    delay(3000);

    while (sendATcommand2("AT+CREG?", "+CREG: 0,1", "+CREG: 0,5", 2000) == 0);//1代表本地上網 5代表漫遊上網
    //SMS while( (sendATcommand("AT+CREG?", "+CREG: 0,1", 500) || 
...
void loop(){
    ...
    {
        // Sets CID parameter
        answer = sendATcommand("AT+HTTPPARA=\"CID\",1", "OK", 5000);//HTTPPARA:設定HTTP的參數
        if (answer == 1)
        {
            // Sets url 
                ...
                if (answer == 1)
                {
                    ...
                    do{
                        ...
                        if (sendATcommand2(aux_str, "+HTTPREAD:", "ERROR", 30000) == 1)
                        {
                            ...
                            do{
                                data_size *= 10;
                                data_size += (aux-0x30);//推測:抓幾個位元組  ， 0x30=數字0
                                while(Serial.available()==0);
                                aux = Serial.read();        
                            }while(aux != 0x0D);//ENTER 的動作
...
int8_t sendATcommand(const char* ATcommand, const char* expected_answer1, unsigned int timeout){
    ...
    do{
        if(Serial.available() != 0){    
            ...
            if (strstr(response, expected_answer1) != NULL) //STRSTR表示兩個輸出參數做比較若後者的值可在前面陣列找到舊不等於NULL   
            {
				printf("\n");
                answer = 1;
            }
